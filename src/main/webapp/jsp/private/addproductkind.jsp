...
<form  id="aaa"class="layui-form" onsubmit="return false">
  <div class="layui-form-item">
    ...
    <div class="layui-input-inline">
      <input type="text" name="code" required  lay-verify="required" placeholder="请输入商品名" autocomplete="off" class="layui-input">
   </div> </div>
  </div>
   <div class="layui-form-item">
    ...
    <div class="layui-input-block">
    <div class="layui-input-inline">
      <input type="text" name="bigparentcode" required  lay-verify="required" placeholder="请输入商品名" autocomplete="off" class="layui-input">
   </div> </div>
  </div>
  <div class="layui-form-item">
    <label class="layui-form-label">名称</label>
    <div class="layui-input-inline">
      <input type="text" name="bigname" required lay-verify="required" placeholder="请输入价格" autocomplete="off" class="layui-input">
    </div>
...
// })
 form.on('submit(shangjia)',function(data){
	 ...
	 $.ajax({
		 url:con.app+'/productcontroller/insert1.do',
		 data:data.field,
		 dataType:'text',
		 type:'Post',
		 success:function(data){
			 console.log("------------------------")
			 if(data=="2"){
				 layer.msg("商品类别已存在请重新输入",{icon:3, time: 800});
			 }if (data =="1") {
